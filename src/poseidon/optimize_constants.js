...
let consts = unstringifyBigInts(require("./poseidon_constants.js"));
...
async function run() {
    // Prime bn128
    const FrBN = new ZqField(Scalar.fromString(primeBN));
    const FrGK = new ZqField(Scalar.fromString(primeGK));
    ...
    for (let i=1; i<=16; i++) {
        const oM = matrix_traspose(consts.bn.M[i-1]);
        const oC =  convertConstants(FrBN, i+1, consts.bn.C[i-1], oM);
        const [oP, oS] = calculatePS(FrBN, i+1, oM);
        opt.bn.C.push(oC);
        opt.bn.M.push(oM);
        opt.bn.P.push(oP);
        opt.bn.S.push(oS);
    ...
    for (let i=1; i<=16; i++) {
        const oM = matrix_traspose(consts.gk.M[i-1]);
        const oC =  convertConstants(FrGK, i+1, consts.gk.C[i-1], oM);
        const [oP, oS] = calculatePS(FrGK, i+1, oM);
        opt.gk.C.push(oC);
        opt.gk.M.push(oM);
        opt.gk.P.push(oP);
        opt.gk.S.push(oS);
    ...
    fs.writeFileSync(path.join( "src", "poseidon", "poseidon_constants_opt.js"), "module.exports = " + JSON.stringify(opt, null, 1), "utf8");
}
