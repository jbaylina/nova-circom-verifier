...
module.exports = async function buildPoseidon(curve) {
    ...
    if (curve == "gk") {
        F = bn128.F1;
        opt = unsringifyConstants(F, poseidonConstants.gk);
    } else if (curve == "bn") {
        F = bn128.Fr;
        opt = unsringifyConstants(F, poseidonConstants.bn);
    } else {
        throw new Error("Curve not specified must be gk for grupkin or bn for the bn128 curve");
    }
